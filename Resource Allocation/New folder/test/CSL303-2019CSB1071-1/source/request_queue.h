...
struct RequestQueue* buildQueue(){
    ...
    queue->front = 0;
    queue->size = 0;
    queue->tail = DEFAULT_SIZE_QUEUE-1;
    queue->Queue = (struct RequestBody*)malloc(DEFAULT_SIZE_QUEUE*sizeof(struct RequestBody));
    return queue;
}
...
int full(struct RequestQueue* queue){
    return (queue->size ==DEFAULT_SIZE_QUEUE);
}
int enqueue(struct RequestQueue* queue, struct RequestBody newRequest){
    if(!full(queue)){
        queue->size = queue->size+1;
        queue->tail = (queue->tail+1)%DEFAULT_SIZE_QUEUE;
...
struct RequestBody dequeue(struct RequestQueue* queue){
    if(!empty(queue)){
        ...
        queue->front = (queue->front+1)%DEFAULT_SIZE_QUEUE;
